...
class Worksheet(xmlwriter.XMLwriter):
    ...
    def set_default_col_width(self, col_width):
        """
        Set the worksheet default column width [integer].

        Args:
            col_width: The default column width.

        Returns:
            Nothing.

        """
        if col_width != self.default_col_width:
            try:
                col_width = int(0.5 + float(col_width))
